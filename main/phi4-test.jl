...
# julia16 main/phi4-test.jl -L 10 -b 0.5 --nsteps 10 -n 1 --wdir=trash/
# julia16 --threads=auto  main/phi4-test.jl -L 80 -b 0.677 -l 0.5 --nmeas 400 --nsteps 18 -n 2000000 --wdir=results/1-Checks/
...
beta    = parsed_args["b"]
lp      = Phi4Parm((lsize_1, lsize_2), beta, lambda)
...
wdir_prefix = "L"*string(lp.iL[1])*"_b"*string(beta)*"_l"*string(lambda)
dt = Dates.now()
...
for i in 1:n_traj
    @time HMC!(phiws, integrator, lp)

    if i % n_meas == 0
        global io_stat = open(magfile, "a")
        write(io_stat, "$(magnetization(phiws.phi))\n")
        close(io_stat)

        global io_stat = open(suscfile, "a")
        write(io_stat, "$(susceptibility(phiws.phi))\n")
        close(io_stat)
