...
- `Lambda_block`
"""
struct CPworkspace{T, N1, N2} <: LFTworkspace
    PRC::Type{T}
    x::Array{T, N1}
    phi::Array{T, N1}
    x_cp::Array{T, N1}
    ...
    Gamma
    Lambda_block
    function CPworkspace(::Type{T}, lp::CPNParm) where {T <: AbstractFloat}
        ...
        return new{T, 3, 5}(T, x, phi, x_cp, J_n, Lambda, frc_x, frc_phi, mom_x, mom_phi, I_N, P_n, Gamma, Lambda_block)
    end
end

function copy!(cpws_dst::CPworkspace, cpws_src::CPworkspace, lp::CPNParm)
    cpws_dst.x .= cpws_src.x
    cpws_dst.phi .= cpws_src.phi
    sync_fields!(cpws_dst, lp)
end

@doc raw"""
    project_to_Sn!(x, lp::CPNParm)

Normalizes the vector field `x` at every lattice point.
"""
function project_to_Sn!(x, lp::CPNParm)
    for j in 1:lp.iL[1]
        for i in 1:lp.iL[2]
...
function randomize_x!(x, lp::CPNParm)
...
"""
function randomize!(cpws::CPworkspace, lp::CPNParm)
...
function fill_Lambda!(cpws::CPworkspace, lp::CPNParm)
    fill_Lambda!(cpws.Lambda, cpws.Lambda_block, cpws.phi, cpws.I_N, cpws, lp)
end

function fill_Lambda!(Lambda, Lambda_block, phi, I_N, cpws::CPworkspace, lp::CPNParm)
...
function fill_Jn!(cpws::CPworkspace, lp::CPNParm)
    fill_Jn!(cpws.J_n, cpws.x, cpws.Lambda, cpws, lp)
end

function fill_Jn!(J_n, x, Lambda, cpws::CPworkspace, lp::CPNParm)
    J_n .= zero(eltype(J_n))
    for j in 1:lp.iL[1]
        for i in 1:lp.iL[2]
            iu = ((i+1) - 1) % lp.iL[1] + 1
...
function mult_add!(vo, A, vi, lp::CPNParm)
    for j in 1:2*lp.N
        for k in 1:2*lp.N
...
"""
    sync_fields!(cpws::CPworkspace, lp::CPNParm)

Updates `cpws.Lambda` and `cpws.J_n` with current `cpws.x` and `cpws.phi`.
"""
function sync_fields!(cpws::CPworkspace, lp::CPNParm)
    fill_Lambda!(cpws, lp)
    fill_Jn!(cpws, lp)
    return nothing
end
