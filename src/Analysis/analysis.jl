...
mutable struct CorrelatorAnalysis <: AbstractCorrelatorAnalysis
    ...
    ID::String
    burnout::Int64
    function CorrelatorAnalysis(filepath::String, type::Type{T1} = AbstractCorrelator; ensemble_ID::String = filepath, burnout::Int64 = 1) where T1 <: AbstractCorrelator
...
function uwrealsym(corrws::AbstractCorrelatorAnalysis)
	...
	uwerr(corrws.ydata[1])
	uwerr(corrws.ydata[end])
...
sign(::Type{T}) where T <: AbstractCorrelator = sign(get_symmetry(T))

function get_symmetry(::Type{T}) where T <: AbstractCorrelator end
function correlator_fit_function(::Type{T}, corrws::AbstractCorrelatorAnalysis) where T <: AbstractCorrelator end
